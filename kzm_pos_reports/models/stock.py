    ...
    def get_shop_location_quantity(self, product_id):
        ...
        if pos_config_obj:
            for each in pos_config_obj:
                ...
                """ % (product_id,
                       " (%s) " % ','.join(map(str, [each.multi_shop_id.location_id.id for each in pos_config_obj if
                                                     each.multi_shop_id])))
            self._cr.execute(sql)
            shop_qty = self._cr.dictfetchall()
            config_list = [i for n, i in enumerate(config_list) if i not in config_list[n + 1:]]
